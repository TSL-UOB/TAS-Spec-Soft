...
\begin{abstract}
Soft robotics is an emerging technology with great potential for commercialisation. Soft gripping is considered to be the most mature area in soft robotics with many instances have been used in industries for various application ranging from automotive to grocery picking.
To accelerate the adoption of soft robots, it is essential that the technology developed must have trustworthiness.
...
\begin{center}
	\begin{tabular}{|p{7mm}|p{72mm}|}
		...
		\hline
		RQ1.5 & The fingers \emph{shall} be inflated with a pressure between X-Y* pressure range. (X, Y to be defined, input number from recycled gripper paper)\\
		\hline
		RQ1.6 & The fingers \emph{shall} be inflated with a flow rate between X-Y* range. (X, Y to be defined, we might have to measure this)\\
		\hline
		...
		\hline
% TODO: could pull these number from SOMA papers or asking Ocado to confirm directly.
	\end{tabular}
\end{center}
