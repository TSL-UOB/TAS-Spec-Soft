...
Here we discuss the importance of developing specifications during development of soft robotic systems and present an extensive example specification for a soft gripper for pick-and-place tasks for grocery items. This specification covers both functional and non-functional requirements, such as reliability, safety, adaptability, predictability, ethics, and regulations.  
We also highlight the need to promote \emph{verifiability} as a first-class objective in the design of a soft gripper.
\todo[size=\tiny]{Shane: Think abstract will need revising once rest of paper polished}
...
%\todo[size=\tiny]{Shane: We need to make case for why specification is important for soft robotics in introduction} DONE
...
\emph{Verifiability} is considered key for improving trustworthiness.  Trustworthiness can be increased  by considering verification as an integral part of system specification and system design \cite{Mousavi2022}, where it can be promoted to a formal design objective \cite{Eder2021}. 
Verifiability will essentially lead to systems that, by their construction, are worthy of our trust.

Soft robotic grippers for manipulating objects are considered to be one of the most mature areas in soft robotics. However, even for this relatively well advanced application area the soft robotics community has so far given very little attention to formulating specifications for soft robotic grippers. Creating a formal specification as part of a development process is an important step in which the requirments for the system are agreed and defined precisely. The specification then provides a benchmark against which to assess different design options. A well defined specification also provides a set of criteria against which to verify the performance of the system during design and in operation.

% where most existing works only describe some parameters of the end-effectors such as their dimensions, weight, and material properties (e.g. \cite{Hong2022,Bhattacharya2019,Tadakuma2020,Loh2014,Nishikawa2019,Mohan2020}).  

%Meanwhile, \emph{verifiability} is considered key for improving trustworthiness.  Trustworthiness can be increased  by considering verification as an integral part of system specification and system design \cite{Mousavi2022}, where it can be promoted to a formal design objective \cite{Eder2021}. 
%Verifiability will essentially lead to systems that, by their construction, are worthy of our trust.
...
Let us consider an automated warehouse, where items are picked from storage crates and are then placed in delivery crates for delivery. An example grocery use case is that of Ocado, which is considered the world's largest online-only supermarket \cite{Triantafyllou2019, Sotiropoulos2018}. 
A range of uncertainties make automation of this process challenging: (i) items can vary in their shape, size, packaging, and orientation; (ii) some items are fragile or deformable; (iii) constrained and relatively cluttered space to operate in; (iv) manufacturing inconsistencies; and (v) elastic nature of the materials used in the gripper, which can lead to performance degradation.
\todo[size=\tiny]{Shane: Not sure about point iii and iv - why are automated warehouses constrained and cluttered? - manufacturing inconsistencies of robots or grocery items?} 
...
\section{Example Specification of a Soft Gripper}\label{specification-gripper}
...
%\todo[size=\tiny]{Shane: Explain how specification developed. Add in explaination of functional vs non-functional requirements} DONE

This example specification was developed through consultation with soft robotic developers, industrial users, ethicists, sociologists and from published requirements. It is an example of the aspects that may need to be specified for a soft gripper rather than a complete specification for every aspect of the system. The specification was developed following the guidance on writing good requirements provided in \cite{NASA2007}. The specification includes functional requirements - those needed to achieve the technical task, and non-functional requirements - those related to the broader operation of the system, such as ethical and regulatory considerations. 
% added by Shane

Below we define the set of reqirements \{RQx\} for a soft gripper across these six properties.
...
\todo[size=\tiny]{Shane: many safety loopholes needs a little bit of explaination}
...
\todo[size=\tiny]{Shane: This doesnt fit pattern of previous sections. I think it needs to be its own section with a bit of explaination for context. Or alternatively for each element to have an example requirement RQ6.x}

Macrae’s \cite{Macrae2022} ‘SOTEC’ (structural, organisational, technological, epistemic, and cultural) framework is a useful approach for identifying domains of sociotechnical risk in autonomous and intelligent systems. The schema can help inform emergent regulation by identifying often-neglected risks. Below we outline each category and consider its application to the pick-and-place tasks of grocery items.
...
\todo[size=\tiny]{Shane: I think the conclusions need revising to emphasise importance of specification - at moment mulitdisciplinary part is rather vague.}
