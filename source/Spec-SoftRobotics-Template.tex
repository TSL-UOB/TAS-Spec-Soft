...
However, the soft robotics community has so far given very little attention to formulating the specifications of soft robotic grippers where most existing works only describe some parameters of the end-effectors like their dimensions, weight, and material (e.g. \cite{Hong2022,Bhattacharya2019,Tadakuma2020,Loh2014,Nishikawa2019,Mohan2020}).  
...
In Sep. 2022, the ASTM International’s committee on robotics, automation, and autonomous systems (F45) formed a new subcommittee on grasping and manipulation, which will develop standards that evaluate performance of grasping type end-effectors.  
...
Human values inform the design and use of technologies, and the resulting distribution of benefits and burdens, hence affecting system trustworthiness. Rather than \textit{describing} whether people \textit{trust} a technology, ethicists \textit{prescribe} whether they \textit{should trust} it because it is (not) trustworthy. Below are the ethics requirements for specifying for trustworthiness in a soft gripper in pick-and-place tasks.
\begin{itemize}
...
Meanwhile, with respect to RQ4.1, one does not need to impose an initial velocity on the food item in the test bed, so this requirement can be met by design (i.e. no need to formally verify or monitor).
...
\begin{table}%[!t]
	...
	\begin{tabular}{|p{20mm}|p{65mm}|}
		...
		\hline
		RQ4.6 & Both Practical tests (functional test), and Measurement (displacement sensor) \\ 
		\hline
	\end{tabular}
...
A key limitation of the state-of-the-art is that they only cover a very limited range of properties, such as performance and adaptability.  
Furthermore, most existing works only describe some parameters of the end-effector, such as their dimensions, weight, and material. 
Therefore, our study, which is explored using a pick-and-place tasks of grocery items, proposed an extensive \emph{specification} for a soft gripper covering several functional and non-functional properties, such as predictability, reliability, adaptability, safety, ethics, and regulations. 
In addition, we promoted the notion of \emph{verifiability} of a soft gripper as a first-class design objective, and identified several design guidelines on formulating verifiability requirements. 
