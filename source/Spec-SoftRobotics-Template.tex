...
Predictability is ``a property of interaction concerning the degree of confidence with which a user can determine the effect subsequent task execution will have on the achievement of the goal"~\cite{Abowd1991}. A soft gripper needs to be predictable in its behaviour to build a degree of confidence for the user, for instance on the expected curving of its fingers when the pump is turned on. 
...
\begin{itemize}
	...
	\item RQ1.5: The fingers \emph{shall} be inflated with a pressure between 3-4 psi pressure range. \footnote{\cite{Partridge2022}}	\item RQ1.6: The fingers \emph{shall} be inflated with a flow rate between 2-3.2 L/min range. \footnote{\cite{DEWIN2022}} 
\end{itemize}
...
\begin{itemize}
	...
	\item RQ2.2: The gripper \emph{shall} hold the item being gripped without dropping it for at least 10 seconds 95\% of the time. \footnote{\cite{Sotiropoulos2018}, pg. 652}
	\item RQ2.3: The gripper \emph{shall} successfully maintain grasp during translation of the gripped item for a maximum velocity and acceleration of 0.03 m/s and 150 mm/s$^2$. \footnote{\cite{Triantafyllou2019}, pg. 5; \cite{Cheng2021}, pg. 15}
	\item RQ2.4: The gripper \emph{shall} successfully grasp when the rate of inflation is in the range of 2-3.2 L/min. \footnote{\cite{DEWIN2022}}
...
However, in this study, we consider safety from the perspective of the item being gripped where physical damage should be avoided during the pick-and-place tasks. % and not from any physical harm to a human.
\begin{itemize}
	...
	\item RQ4.4: When grasping a hard–fragile item (e.g. light bulb, raw egg), the soft actuator \emph{shall} be inflated until the gripping force does not exceed 2N. \footnote{\cite{Cheng2021}, pg. 14}
	\item RQ4.5: When grasping a soft–fragile item like cake or bread, the soft actuator \emph{shall} be inflated until the fingertip displacement does not exceed 3mm. \footnote{\cite{Cheng2021}, pg. 14}
	\item RQ4.6: When grasping a soft–fragile item like strawberry or bayberry, the soft actuator \emph{shall} be inflated until the gripping force does not exceed 1N and the fingertip displacement does not exceed 1mm. \footnote{\cite{Cheng2021}, pg. 14}
\end{itemize}
%\begin{center}
%	\begin{tabular}{|p{7mm}|p{72mm}|}
%		\hline
...
The notion of \emph{verifiability} is considered key for improving trustworthiness of an AS, such as a soft robot \cite{Mousavi2022}. 
A unified and holistic approach to verifiability will fundamentally change the approach to verification of ASs, which will lead to systems that are by their construction worthy of our trust. 
...
In our study, we have considered this early during specification, by taking into account how these properties can be verified later (see Table~\ref{Table:Verifiability}). 
...
With respect to these i--iii challenges, our study has taken key steps towards addressing them. We propose a wide-ranging specification of requirements to demonstrate trustworthiness (Section III); and the specification includes requirements that adapts the soft gripper to objects of different sizes, shapes, masses and orientations/positions.

Most of the requirements in Section III are provided in a positive tone (avoiding negative requirements) along with any quantifiable performance measurements. 
But, according to \cite{Gerson1993}, writing requirements for verifiability goes far beyond this where system engineers must design for verifiability. For example, these could be bounding the verification task, prioritizing effort, ensuring traceability and breaking down high-level requirements into verifiable portions. 

Some requirements in the specification have been specified are high-level requirements, which needs to be extended into verifiable portions. 
For instance, RQ1.1 can be broken by specifying the amount of curvature expected in mm or as a percentage of the diameter. 
Similarly, RQ1.2 can mention how much degrees a finger needs to straighten when the pump is turned off. 

Requirement RQ 4.1 can be verified using a vision camera to inspect whether the item is moving or not. 
RQ4.2 can be verified using a force torque sensor and 4.3 using both force torque sensor and vision camera. 
