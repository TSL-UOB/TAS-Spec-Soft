...
\begin{figure*}
	\centering
	\includegraphics[width=0.9\textwidth]{figures/soft-t.png}
...
Ethics aims at identifying right action(s) in specific situations. It is relevant to this paper because human values inform the design and use of technologies, and the resulting distribution of benefits and burdens, hence affecting system trustworthiness. Ethicists are not interested in \textit{describing} whether people \textit{trust} a technology, but in \textit{prescribing} whether they \textit{should trust} it because it is (not) trustworthy. Below are the ethics requirements for specifying for trustworthiness in a soft gripper.
\begin{itemize}
	\item RQ5.1: In pick-and-place 
        tasks, the gripper \emph{shall} be 
        environmentally sustainable. 
        \item RQ5.2: In pick-and-place tasks, the gripper \emph{shall} avoid distressing human workers.
        \item RQ5.3: In pick-and-place tasks, the gripper \emph{shall} avoid exploiting humans’ trust heuristics.
        \item RQ5.4: The gripper \emph{shall} allow fair treatment of current human pickers.
\end{itemize}

The use of a recycled gripper may make its manufacturers or the company deploying it \textit{look like} they care for the environment and so appear more trustworthy, thus increasing public trust in them. However, such a system may not perform as well as, or may degrade more rapidly than, a system made of virgin material, hence making it more likely for items to be dropped or damaged. When the item being gripped is food, the risk of food waste increases, with detrimental implications for the environment. Unless the above reliability requirements  are met by design, the use of a recycled gripper could be a form of “greenwashing” \cite{delmas2011drivers}.    

Moreover, while technical requirements aim to prevent \textit{physical} harm, specifying for trustworthiness requires that considerations of system safety expand to potential \textit{psychological} harms \cite{Porter2022}. By replicating a human feature (fingers/hand) and behaviour (gripping), a soft gripper risks producing an uncanny valley effect \cite{moore2012bayesian}, which causes humans to feel distressed in interactions with robots that mimic, but are not close enough to, human or zoomorphic features and demeanours. User studies should be undertaken to identify gripper shapes that are acceptable to humans, whilst also aiming for optimal gripping.  
	
 
Simultaneously, because of their biomimetic features, soft robots risk exploiting humans’ trust heuristics, i.e. cognitive shortcuts that make humans trust things they are already familiar with \cite{Manzini}. Because soft robots present many safety loopholes \cite{abidi2017intrinsic}, trust in the safety of a soft gripper could be misplaced if it was only based on a sense of familiarity with it. To avoid posing humans at risk of harm, the robot area should be clearly separated from the space accessible to them.

 
Finally, increase in warehouse automation through the deployment of soft grippers may improve efficiency in grocery settings, in turn leading to decreased costs to the advantage of consumers; it could also relieve workers from routine or physically demanding tasks. However, in the transition to increased automation current human pickers may be treated unfairly, as automation may reduce employment opportunities or dramatically change the skills required of employees. Company policies should be implemented to retrain or upskill current workers \cite{georgieff2021happened, nedelkoska2018automation}.
...
\emph{Cultural} sources of risk arise from collective values (beliefs and norms) that frame and influence AIS design and operation. For instance, if a gripper’s grasp is optimized for western food items (e.g., tins), this may mean that it underperforms for eastern food items (e.g., bags) that only affect a minority of stakeholders. Regulatory requirements must reflect critically on their underlying values, and work to counter potential inequities. 
...
On one hand, most requirements of the specification contain numeric tolerances. 
...
On the other hand, there are several high-level requirements in the specification, which need to be refined into verifiable portions before any verification method can be applied. 
%Following the guidance provided in \cite{NASA2007} on how to write good requirements, engineering requirements in Section~\ref{specification-gripper} have been formulated positively (i.e. use of \emph{shall} statements as opposed to \emph{shall not} statements). 
...
This can be verified by monitoring the distance between each of the joints of the gripping system to ensure there is no collision between the body of the gripping system and the item. 
Meanwhile, with respect to RQ4.1, we don't need to impose an initial velocity on the food item in the test bed, so this requirement can be met by design (i.e. no need to formally verify or monitor).
...
Thus, as seen here, our study provides two versions of the specification -- one a \emph{verifiable} one from the start, and the other a more \emph{high-level} one, for which by using examples we demonstrate how to make it verifiable. 
By doing this, we show what is the difference or delta between a high-level requirement and its verifiable version. 
In this manner, our study not only provides a wide-ranging specification for a pick-and-place application, but it also goes a step further by providing design guidelines on formulating verifiable requirements for the soft robotics community. % requirements verifiable.
